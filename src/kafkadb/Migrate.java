...
public class Migrate {
	...
	public static TransMeta makeTrans(String source, String target,
		...
		String selectSQL = "SELECT * FROM " + source;// +" WHERE id!=1";
		...
		if (filename != null) {
			...
			for (Iterator<String> it = filename.iterator(); it.hasNext();) {
				...
				for (Iterator<StepMeta> it2 = s.iterator(); it2.hasNext();) {
					...
					if (sp.getName().startsWith("depends-")) {
						depends.add(sp.getName().split("-")[1]);
					}
			...
			dependencies.put(transMeta, depends);
		}
	...
	public static void writeFile() {
		try {
			...
			FileWriter fstream = new FileWriter("/tmp/out.sql");
